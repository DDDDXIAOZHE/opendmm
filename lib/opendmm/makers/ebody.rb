    ...
    module EBody
      ...
      module Parser
        def self.parse(content)
          ...
          return {
            actresses:     specs['出演女優'].css('a').map(&:text).map(&:squish),
            code:          specs_rhs['品番'],
            cover_image:   URI.join(page_uri, html.css('div.package > a.package-pic').first["href"]).to_s,
            description:   html.css('div.title-data > p.comment').text.squish,
            ...
            series:        specs['シリーズ'].text.remove('：').discard_if_empty,
            title:         html.css('div.title-data > h1').text.squish,
          }
        end
      end
    end
